...
class ASEToolFix(BuiltinTool):
    def _invoke(self, 
        ...
        try:
            logging.info(f"Fix ASE Recieved {tool_parameters}")

            ase_inputs = ASEToolModel(**tool_parameters)
            response = trigger_ase("maint_fix_do", ase_inputs.dict())
            return self.create_text_message(text=response)
        except Exception as e:
            logging.info(f"Traceback: {traceback.format_exc()}")
            raise Exception(f"Failed Fix Run. {e}")