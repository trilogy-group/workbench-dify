...
class ASEToolRCAQC(BuiltinTool):
    def _invoke(self, 
                user_id: str,
        ...
        try:
            logging.info(f"RCA QC ASE Recieved {tool_parameters}")

            ase_inputs = ASEToolModel(**tool_parameters)
            response = trigger_ase("maint_rca_qc", ase_inputs.dict())
            return self.create_text_message(text=response)
        except Exception as e:
            logging.info(f"Traceback: {traceback.format_exc()}")
            raise Exception(f"FAILED RCA QC RUN. {e}")